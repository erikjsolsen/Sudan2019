...
#setwd("~/github/sudan")
setwd("/Users/eriko/ownCloud/R/Sudan/Sudan-master")
...
setwd("~/ownCloud/R/Sudan/Sudan-master/sudan_management_areas")
...
#' ----------------------------------
trophic.plot <- ggplot(subset(catch.traits, TGShort!="NA"), aes(TGShort, CPUEw/NArea))
...
setwd("~/ownCloud/Research/Sudan/Distribution diversity LW paper/PlosOne/new figures")

tiff(file="fig 5 trophic_plot2.tiff", width=3000, height=3000, res=400, pointsize=10, compression=c("none"))
trophic.plot
dev.off()


#' PLOT CPUE by depth plot - traps,  by area - FIG 6
#' ---------------------------------
depth.plot <- ggplot(subset(subset(catch.traits, gear=="TB"), TGShort!="NA"), aes(depth, CPUEw)) + geom_point(aes(colour=TGShort))  + labs(x="Depth (m)", y="CPUE (kg(hrs)") + scale_fill_manual(name="Management area",values=sequential_hcl(n = 7, h = c(0, -100), c = c(80, NA, 40), l = c(53, 75), power = c(1, 1), rev = TRUE, register = ), guide=FALSE ) +theme_bw()  +facet_grid(vars(id),vars(TGShort))

depth.plot

tiff(file="fig 6 CPUE depth_plot.tiff", width=3000, height=3000, res=400, pointsize=10, compression=c("none"))
depth.plot
dev.off()
...
tiff(file="fig 1 sudan_bathy_v2.tiff", width=1900, height=3000, res=400, pointsize=10, compression=c("none"))
...
text(37.4, 20.92, "2. Dungonab", cex=0.9)
