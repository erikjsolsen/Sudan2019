...
```{r bathymap sudan, echo=FALSE, eval=FALSE}
# OLD CODE - Not Run 
...
## Bathymetric map with ggplot and marmap
New code for making bathymetric map to include depth legend, north arrow and scale bar. 
...
duration.plot.gn <-  duration.plot.gn +  geom_point() + stat_summary(fun.y=mean, geom="point", shape=20, size=3, color="red", fill="red") + labs( x="Management areas", y="Hours fishing (gillnets)")  +theme_bw() +facet_wrap(~survey_m, labeller=labeller(survey_m=c("Survey 1: Nov. 2012" = "Nov.2012",
      "Survey 2: May 2013" = "May.2013",
      "Survey 3: Nov. 2013" = "Nov.2013")))
...
gear.plot <- c4 %>%  mutate(FamGroup2 = fct_relevel(FamGroup2,  "Other Spp", "Serranidae","Scombridae","Lutjanidae", "Lethrinidae", "Chirocentridae", "Carcharhinidae", "Carangidae",  "Acanthuridae")) %>%
 ggplot( aes(FamGroup2, fill=survey_m)) + geom_bar(binwidt=0.2, aes( weight=CPUEw)) +theme_bw() + facet_grid(vars(gear), vars(survey_m), labeller=labeller(survey_m=c("Survey 1: Nov. 2012" = "Nov.2012",
      "Survey 2: May 2013" = "May 2013",
      "Survey 3: Nov. 2013" = "Nov.2013"), gear=c("GN" ="Gillnet", "TB" ="Trap"))) + coord_flip() +xlab("") + ylab("kg/hour fishing (cumulative)")  + guides(fill=guide_legend(title="Survey")) + theme(legend.position = "none") + guides(fill=FALSE) 
gear.plot
...
# By trophic group 
# CPUEw
c5 <- subset(c4, CPUEw>0)[,c(1,17, 31, 25, 40)]
...
p2<-p2+ geom_text(size=3,position=position_jitter(width=0.1,height=0.1)) +  expand_limits(x = c(-0.12, 1.12), y=c(-1.12, 0.12)) +
  xlab(percentage[1]) + 
...
(gear.plot2 +p) / (gear.plot+ p2) + plot_annotation(tag_levels = 'A') & 
  theme(plot.tag = element_text(size = 10))

#gear.plot2| pw3 

ggsave("./figs/Fig4_gearplot PCA.tiff", width=8, height=6)
ggsave("./figs/Fig4_gearplot PCA.png", width=8, height=6)
```
