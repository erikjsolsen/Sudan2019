...
catch_pr_cell<-group_by(catch,x,y) %.% 
  summarise(mean=mean(W_CPUE),total=sum(W_CPUE))

#top 5 fish
top_catch_pr_cell<-group_by(topfish,Sci_name, survey,x,y) %.% 
  summarise(mean=mean(W_CPUE),total=sum(W_CPUE))


#by species families
species_catch_pr_cell<-group_by(catch,survey,Fam_name,x,y) %.% 
  summarise(mean=mean(W_CPUE),total=sum(W_CPUE))

#by CPUE, species families & gear type
TBspecies_catch_pr_cell<-group_by(TBcpue,survey,Fam_name,x,y) %.% 
  summarise(mean=mean(W_CPUE), total=sum(W_CPUE))

HLspecies_catch_pr_cell<-group_by(HLcpue,survey,Fam_name,x,y) %.% 
  summarise(mean=mean(W_CPUE), total=sum(W_CPUE))

GNspecies_catch_pr_cell<-group_by(GNcpue,survey,Fam_name,x,y) %.% 
  summarise(mean=mean(W_CPUE), total=sum(W_CPUE))
...
catch_pr_cell<-cbind(catch_pr_cell, ALL_catVar)
...
top_catch_pr_cell<-cbind(top_catch_pr_cell, top_catVar)
...
TBspecies_catch_pr_cell<-cbind(TBspecies_catch_pr_cell, TB_catVar)
...
HLspecies_catch_pr_cell<-cbind(HLspecies_catch_pr_cell, HL_catVar)
...
GNspecies_catch_pr_cell<-cbind(GNspecies_catch_pr_cell, GN_catVar)
...
gridmap<-ggplot(plotdata1) + geom_tile(aes(x,y,fill=factor(ALL_catVar))) + scale_fill_brewer(palette="OrRd", labels=il) + labs(x = "LON", y = "LAT", fill = "kg/hr fishing") + ggtitle("CPUE all GEAR and all SURVEYS combined")
...
top5_map + theme_bw() + geom_polygon(data=sudanmap3, aes(x=long, y=lat, group=group, fill=group), fill="gray52")
...
TB_map<-ggplot(TBplotdata[TBplotdata$Fam_name %in% facets,]) + theme_bw() + geom_tile(aes(x,y,fill=factor(TB_catVar))) + scale_fill_brewer(palette="OrRd", labels=il)  + labs(x = "LON", y = "LAT", fill = "kg/hr fishing") + ggtitle("CPUE Traps - by survey") + facet_wrap(~Fam_name + survey)

#combining grid maps w coastline
#TB2<-TBplotdata[1:2]
TB_map + geom_polygon(data=sudanmap3, aes(x=long, y=lat, group=group, fill=group), fill="gray90")
ggsave("TB_CPUE_by_survey_facets.pdf", scale = 1, width=7.78, height=10, dpi = 400)
...
#plotting only select variables - facetting - by SURVEY
facets<-c("LETHRINIDAE", "LUTJANIDAE", "SERRANIDAE")
HL_map<-ggplot(HLplotdata[HLplotdata$Fam_name %in% facets,]) + theme_bw() + geom_tile(aes(x,y,fill=factor(HL_catVar))) + scale_fill_brewer(palette="OrRd", labels=il)  + labs(x = "LON", y = "LAT", fill = "kg/hr fishing") + ggtitle("CPUE Hand-Line - by survey") + facet_wrap(~Fam_name + survey)

#combining grid maps w coastline
HL_map  + geom_polygon(data=sudanmap3, aes(x=long, y=lat, group=group, fill=group), fill="gray90")
ggsave("HL_CPUE_by_survey_facets.pdf", scale = 1, width=7.78, height=10, dpi = 400)
...
GN_map<-ggplot(GNplotdata[GNplotdata$Fam_name %in% facets,]) + theme_bw() + geom_tile(aes(x,y,fill=factor(GN_catVar))) + scale_fill_brewer(palette="OrRd", labels=il)  + labs(x = "LON", y = "LAT", fill = "kg/hr fishing") + ggtitle("CPUE Gillnet - by survey") + facet_wrap(~Fam_name + survey)

#combining grid maps w coastline
GN_map  + geom_polygon(data=sudanmap3, aes(x=long, y=lat, group=group, fill=group), fill="gray90")
ggsave("GN_CPUE_by_survey_facets.pdf", scale = 1, width=7.78, height=10, dpi = 400)